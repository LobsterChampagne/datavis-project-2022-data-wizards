...
// Parse the Data
d3.csv("./Data/platform_per_year.csv").then(function (dd) {
	//get distinct years
	...
	$("#year_plot")
		...
		.on("input", function () {
			...
			if (year == 2023) {
				$("#year_txt").val("Today (2022 + undated)");
			} else {
				$("#year_txt").val(
					"Year " + document.getElementById("year_plot").value
				);
	...
	function draw_barplot(year) {
		...
		data = dd.filter(
			(d) =>
				(d.date_added <= year && d.date_added != 2000) || year == 2023
		);
		...
		const y = d3
			...
			.domain([
				...
				d3.max(
					data.map(
						(d) =>
							d.MovieOld + d.MovieNew + d.TVShowNew + d.TVShowOld
					)
				),
			])
			.range([height, 0]);
